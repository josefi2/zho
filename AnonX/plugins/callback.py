...
async def unban_assistant_(_, CallbackQuery):
    ...
    if not a.can_restrict_members:
        return await CallbackQuery.answer(
            "⌯︙ما عندي صلاحيات حته افك الحظر عن الحساب المساعد",
            show_alert=True,
        )
    else:
        try:
            ...
            return await CallbackQuery.answer(
                "⌯︙ حدث خطا في الغاء حظر الحساب المساعد.",
                show_alert=True,
            )
    ...
    if command == "Pause":
        ...
        if int(command) in [1, 3]:
            if (duration_played - duration_to_skip) <= 10:
                ...
                return await CallbackQuery.answer(
                    f"⌯︙البوت غير قادر على التقديم .\n\n⌯︙التشغيل الحالي :** {bet}** دقيقه من اصل **{duration}** دقيقه.",
                    show_alert=True,
                )
