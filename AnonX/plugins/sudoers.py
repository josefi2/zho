...
@app.on_message(
    command(["ثانوي"]) & filters.user(OWNER_ID)
)
@language
async def useradd(client, message: Message, _):
    if MONGO_DB_URI is None:
        return await message.reply_text(
            f"**⌯︙مشكله في {MUSIC_BOT_NAME}'لا تستطيع اضافة مطورين الى قاعدة بينات {MUSIC_BOT_NAME} .\n\n ⌯︙قم باضافة قاعدة بيانات لاستعمال هذه الميزه.**"
        )
...
@app.on_message(
    command(["حذف ثانوي"]) & filters.user(OWNER_ID)
)
@language
async def userdel(client, message: Message, _):
    if MONGO_DB_URI is None:
        return await message.reply_text(
            f"**⌯︙مشكلة في {MUSIC_BOT_NAME}'لا تستطيع اظافة مطورين الى قاعدة بينات {MUSIC_BOT_NAME} .\n\n ⌯︙قم باضافة قاعدة بينات لاستعمال هذه الميزه.**"
        )
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text(_["auth_1"])
        ...
        if removed:
            ...
            return
        await message.reply_text(f"⌯︙حدث خطا.")
        return
...
@app.on_message(command(["الثانويين"]) & ~BANNED_USERS)
@language
