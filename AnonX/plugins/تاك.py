...
array = []
@app.on_message(command(["هولي جيبيهم", "هولي تاك للكل","جيبيهم هولي"]) & ~filters.private)
async def nummmm(client: app, message):
  ...
  if not chek.status in ["administrator", "creator"]:
    ...
    return
  await message.reply_text("**جاري بدأ التاك ، لايقاف الامر اضغط **\n /cancel او اكتب ايقاف التاك")
  i = 0
  txt = ""
  zz = message.text
  if message.photo:
          ...
          zz = message.caption
  try:
   zz = zz.replace("جيبيهم هولي","").replace("هولي جيبيهم","").replace("هولي تاك للكل","")
  except:
    pass
...
@app.on_message(command(["ايقاف التاك", "/cancel","بس هولي"]))
